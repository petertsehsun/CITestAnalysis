...
parser.add_argument('--dbName', help='Name of the database which stores the test results', type=str, required=True)
...
def compareTests(testsToCompare):
    ...
    for tid, test in combinedTests.items():
        ...
        if len(test) < 2: # the test is only executed in one DB
            continue
